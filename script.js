...
// Функция для полной блокировки скролла на мобильных
function preventDefault(e) {
    e.preventDefault();
}

// Включение/выключение блокировки
function toggleScroll(isMenuOpen) {
    if (isMenuOpen) {
        document.body.classList.add('no-scroll');
        // Блокируем прокрутку колесом мыши и стрелками
        window.addEventListener('wheel', preventDefault, { passive: false });
        // Блокируем тач-скролл (свайпы пальцем) на смартфонах
...
menuLinks.forEach(link => {
    link.addEventListener('click', () => {
        ...
        toggleScroll(false); // Всегда возвращаем скролл сайту
    });
});
